...
describe('Provision Node driver RKE1 cluster with AWS', { testIsolation: 'off', tags: ['@debug', '@manager', '@adminUser', '@standardUser', '@jenkins'] }, () => {
  ...
  it('can create a rke1 cluster with Amazon cloud provider', function() {
    ...
    createRKE1ClusterPage.rke1PageTitle().should('contain', 'Amazon EC2');
  ...
  it('can add a node to the cluster', function() {
    ...
    // save changes
    cy.intercept('PUT', `v3/clusters/${ clusterId }?_replace=true`).as('saveRke1Cluster');
    ...
    clusterDetails.machinePoolsList().resourceTable().sortableTable().groupElementWithName(`${ this.nodeName }-2`)
      .next('tr.main-row', { timeout: 700000 })
