...
describe('GitHub', { testIsolation: 'off', tags: ['@adminUser', '@usersAndAuths'] }, () => {
  before(() => {
    cy.login();
  ...
  it('Configure OAuth App in GitHub', () => {
    ...
    cy.get('a[href="/settings/applications/new"]').contains('New OAuth app').click();
    cy.url().should('include', 'github.com/settings/applications/new');
  ...
  it('GitHub Auth Login/Logout screen', () => {
    authProviderPo.goTo();
    githubPo.waitForPage();
    ...
    // check success message on screen
