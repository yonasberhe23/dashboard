import { FleetClusterDetailsPo, FleetClusterListPagePo } from '@/cypress/e2e/po/pages/fleet/fleet.cattle.io.cluster.po';
...
import { generateFakeFleetClusterAndIntercepts } from '~/cypress/e2e/blueprints/fleet/fake-fleet-cluster';
...
describe('Fleet Clusters', { tags: ['@fleet', '@adminUser'] }, () => {
  ...
  describe('bundle manifests are deployed from the BundleDeployment into the downstream cluster', { tags: ['@fleet', '@adminUser', '@jenkins'] }, () => {
    const region = 'us-west-1';
    const namespace = 'fleet-default';
    const removeCluster = false;
    const removeGitRepo = false;
    const removeWorkspace = false;
    ...
    beforeEach(() => {
      ...
      cy.createE2EResourceName('cluster').then((name) => {
        clusterName = name;
      });
    ...
    it('testing fake cluster', () => {
      generateFakeFleetClusterAndIntercepts(clusterName);
      ...
      fleetClusterListPage.resourceTableDetails(clusterName, 1).contains('Active');
      // check bundles ready
      ...
      fleetClusterListPage.resourceTableDetails(clusterName, 5).contains( ' 15 ');
