...
# SSH keys
corral_user_public_key: |
$(cat ${PRIV_KEY}.pub | sed 's/^/  /')
corral_user_id: jenkins
EOF
...
if [ ! -d "${QA_INFRA_REPO}" ]; then
    ...
    echo "Please set QA_INFRA_REPO environment variable or clone qa-infra-automation repo"
    exit 1
fi

# Run Ansible playbook
ansible-playbook \
  -i "${QA_INFRA_REPO}/ansible/ui-dashboard-tests/inventory-template.yml" \
...
# Extract node IP from Ansible output (if needed for downstream jobs)
# This will depend on how the Ansible playbook outputs the information
# For now, we'll check if there's a generated output file
