 ...
 *
 * Required env vars:
 *   GITHUB_TOKEN       — PAT with repo + project scopes (or Vault-sourced token)
 *   JENKINS_AUTH       — base64-encoded "username:token" for Jenkins REST API
 *   JENKINS_BASE_URL   — Jenkins instance base URL
 *
 * Optional env vars:
 *   JENKINS_JOB_PATH, GITHUB_ORG, GITHUB_REPO, GITHUB_PROJECT_NUMBER,
 *   GITHUB_PROJECT_OWNER, GITHUB_ISSUE_LABELS, STATUS_FIELD_ID, BACKLOG_OPTION_ID,
 *   DRY_RUN
 */
  ...
  for (const failure of grouped.values()) {
    try {
      ...
      if (existing?.state === 'open') {
        // Issue is already being tracked — just add a comment with the latest env context
        if (failure.environments.length > 0) {
          await githubClient.addEnvironmentComment(existing.id, failure.environments);
        }
