...
// Get the Kubernetes releases per distrobution type
Cypress.Commands.add('getKubernetesReleases', (distroType: 'rke2' | 'k3s') => {
  return cy.request({
    method:  'GET',
    url:     `${ Cypress.env('api') }/v1-${ distroType }-release/releases`,
    headers: {
      'x-api-csrf': token.value,
      Accept:       'application/json'
    },
...
Cypress.Commands.add('createAmazonRke2Cluster', (params: CreateAmazonRke2ClusterParams) => {
  ...
  return cy.getKubernetesReleases('rke2').then((resp: Cypress.Response<any>) => {
    const length = resp.body.data.length - 1;

    const latestKubernetesVersion = resp.body.data[length].id;
